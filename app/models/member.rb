class Member < ApplicationRecord
  ...
  validates :name, format: { with: /\A[^0-9`!@#\$%\^&*+_=]+\z/, message: "only allows letters" }, uniqueness: true, if: "name.present?"
  validates_presence_of :heading
  validates :heading, length: { minimum: 15 }, if: "heading.present?"
  validates_presence_of :website
  validates :website, format: { with: /\A(http|https):\/\/[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?\z/ix, message: "is not a valid URL" }, uniqueness: true, if: "website.present?"
